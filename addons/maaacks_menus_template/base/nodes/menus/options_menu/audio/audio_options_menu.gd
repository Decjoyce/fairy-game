...
func _add_audio_control(bus_name : String, bus_value : float, bus_iter : int, itep: int = -1) -> void:
	...
	if audio_control is OptionControl:
		...
		if itep == 0:
			audio_control.get_child(1).grab_focus()
...
func _add_audio_bus_controls() -> void:
	...
	for bus_iter in AudioServer.bus_count:
		...
		_add_audio_control(bus_name, linear, bus_iter, it)
