...
@export var alt_positions_to_move_to: Array[Node3D]
...
func move_item_from_index(sig: float = -1) -> void:
...
func move_all_items(sig: float = -1) -> void:
	assert(positions_to_move_to[0], "ITM_MOVER: No positions assigned")
...
func smooth_move_all_items(sig: float = -1) -> void:
	assert(positions_to_move_to[0], "ITM_MOVER: No positions assigned")
	tween = get_tree().create_tween().bind_node(self).set_trans(Tween.TRANS_LINEAR)
	tween.set_parallel(true)
	for i in itms_2_move.size():
...
func alt_move_item_from_index(sig: float = -1) -> void:
	assert(alt_positions_to_move_to[0], "ITM_MOVER: No positions assigned")
	assert(itms_2_move[0], "ITM_MOVER: No items assigned")
	
	var new_pos: Node3D = alt_positions_to_move_to[0]
	if alt_positions_to_move_to[cur_item_index]: new_pos = alt_positions_to_move_to[cur_item_index]
	itms_2_move[cur_item_index].global_position = new_pos.global_position
	itms_2_move[cur_item_index].global_rotation = new_pos.global_rotation

func alt_smooth_move_item_from_index(sig: float = -1) -> void:
	assert(alt_positions_to_move_to[0], "ITM_MOVER: No positions assigned")
	assert(itms_2_move[0], "ITM_MOVER: No items assigned")
...
func alt_move_all_items(sig: float = -1) -> void:
	assert(alt_positions_to_move_to[0], "ITM_MOVER: No positions assigned")
	for i in itms_2_move.size():
		if !itms_2_move[i]: continue
		var new_pos: Node3D = alt_positions_to_move_to[0]
		if alt_positions_to_move_to[i]: new_pos = alt_positions_to_move_to[i]
		itms_2_move[i].global_position = new_pos.global_position
		itms_2_move[i].global_rotation = new_pos.global_rotation

func alt_smooth_move_all_items(sig: float = -1) -> void:
	assert(alt_positions_to_move_to[0], "ITM_MOVER: No positions assigned")
	tween = get_tree().create_tween().bind_node(self).set_trans(Tween.TRANS_LINEAR)
	tween.set_parallel(true)
	for i in itms_2_move.size():
